...
class MyDisplay(Display):
    ...
    def _do(self, problem: object, evaluator: object, algorithm: object):
        ...
        # fitness = (outdated_servers_being_used, maximum_time_spent_relocating_services, delay_sla_violations)

        outdated_servers_being_used = int(np.min(algorithm.pop.get("F")[:, 0]))
        maximum_time_spent_relocating_services = int(np.min(algorithm.pop.get("F")[:, 1]))
        delay_sla_violations = int(np.min(algorithm.pop.get("F")[:, 2]))

        # Aggregating penalties
        penalty = int(np.min(algorithm.pop.get("CV")[:, 0]))

        self.output.append("OUTD", outdated_servers_being_used)
        self.output.append("MIGR", maximum_time_spent_relocating_services)
...
class PlacementProblem(Problem):
    ...
    def get_fitness_score_and_constraints(self, solution) -> tuple:
        ...
        if VERBOSE:
            ...
            print(f"\tOutdated hosts being used: {outdated_servers_being_used}")
...
def get_migration_plan(pop_size, cross_prob, mut_prob, n_gen) -> list:
    ...
    problem = PlacementProblem(services_to_migrate=[service for service in Service.all() if service.server.status == "outdated"])
    res = minimize(
        problem,
        method,
