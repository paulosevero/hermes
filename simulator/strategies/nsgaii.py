...
class MyDisplay(Display):
    ...
    def _do(self, problem: object, evaluator: object, algorithm: object):
        ...
        avg_outdated_servers_being_used = round(np.average(algorithm.pop.get("F")[:, 0]), 2)
        ...
        avg_time_spent_relocating_services = round(np.average(algorithm.pop.get("F")[:, 1]), 2)
        ...
        avg_delay_sla_violations = round(np.average(algorithm.pop.get("F")[:, 2]), 2)
        ...
        self.output.append("AVG_OUTD", avg_outdated_servers_being_used)
        ...
        self.output.append("AVG_MIGR", avg_time_spent_relocating_services)
        self.output.append("MAX_MIGR", max_time_spent_relocating_services)
        self.output.append("MIN_SLAV", min_delay_sla_violations)
        self.output.append("AVG_SLAV", avg_delay_sla_violations)
...
class PlacementProblem(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        """
        solutions_output = [self.get_fitness_score_and_constraints(solution=solution) for solution in x]

        parsed_output = []

        for solution_output in solutions_output:
            fitness_values = (
                solution_output["outdated_capacity_prevented_from_being_drained"],
                solution_output["maximum_time_spent_relocating_services"],
                solution_output["delay_sla_violations"],
            )

            parsed_output.append((fitness_values, solution_output["penalty"]))

        out["F"] = np.array([item[0] for item in parsed_output])
        out["G"] = np.array([item[1] for item in parsed_output])
    ...
    def get_fitness_score_and_constraints(self, solution) -> tuple:
        ...
        output = {
            "outdated_capacity_prevented_from_being_drained": outdated_capacity_prevented_from_being_drained,
            "maximum_time_spent_relocating_services": maximum_time_spent_relocating_services,
